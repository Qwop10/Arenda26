...
router.patch('/:id/status', async (req, res) => {
  try {
    const { rows } = await pool.query(
      'UPDATE transfer_bookings SET status=$1 WHERE id=$2 RETURNING *',
...
router.patch('/:id/receipt', async (req, res) => {
  try {
    const { rows } = await pool.query(
      'UPDATE transfer_bookings SET receipt_url=$1 WHERE id=$2 RETURNING *',
      [req.body.receipt_url, req.params.id]
    );
    res.json(rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
